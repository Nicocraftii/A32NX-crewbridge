...
class MobiFlightVariableRequests:
    ...
    def get(self, variableString):
        if variableString not in self.sim_var_name_to_id:
            ...
            # ─────────────── Do NOT wait here ───────────────
            # The initial value will arrive asynchronously via callback soon after

        variable_id = self.sim_var_name_to_id[variableString]
        sim_var = self.sim_vars[variable_id]

        # Optional: very short safety wait (e.g. 50–100 ms total)
        wait_counter = 0
        while wait_counter < 10 and sim_var.float_value is None:
            sleep(0.0005)
            wait_counter += 1

        if sim_var.float_value is None:
            # Either return None, or 0.0 — but do NOT block forever
            return 0.0
